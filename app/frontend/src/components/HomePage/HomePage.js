            ...
            <Slider className="slider-wrapper" autoplay={2500} infinite={true}>
                {slides.map((slide, index) =>
                    <div key={index} className="inner slider-content"
                         style={{backgroundImage: "url(" + slide.bg_images + ")"}}>

                        {index % 2 === 1 && <div style={{backgroundColor: colorBG}}>
                        ...
                        {index % 2 !== 1 && <div style={{backgroundColor: colorLightBG, padding: 30}}><h1
                            style={{fontSize: "45px", color: colorAccent}}>Join the Trading Revolution!
                        </h1></div>}
                        <br/>
                        {index % 2 !== 1 && <p style={{
                            color: "#FFFFFF", fontSize: "25px", textAlign: "center", lineHeight: 1.5, paddingLeft: 400
                            , paddingRight: 400,
                        }}>{slide.description}</p>}
                        <br/>
                        {!user || !user.loggedIn ? <button style={{width: 250, color: "white"}} name="sign_up"
                                                           onClick={this.navigateSignUp}>Get Started Now</button> :
                            <button style={{width: 250, color: "white"}} name="trading-equipment"
                                    onClick={this.navigateEvents}>Start Investing</button>}
